...
import { AspirantBachillerateService } from 'src/app/service/pdf/aspirantBachiller/aspirant-bachillerate.service';
...
@Component({
  selector: 'app-formulario-bachiller',
  templateUrl: './formulario-bachiller.component.html',
  styleUrls: ['./formulario-bachiller.component.css']
})
export class FormularioBachillerComponent {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          let asp = this.serviceAspirantService.postUpdate(aspirantPost).subscribe(
            (valor: number) => {
              let aspirantBachillerate: AspirantBachillerateSave = {
                'schoolOrigin': `${this.formData.escProcAspA}`,
              ...
              this.serviceViewAspirantBachillerateService.postAspirantBachillerate(aspirantBachillerate).subscribe(
                (idAspBasic: number) => {
                  this.aspirantBachillerateService.getDataPDF(idAspBasic).subscribe(response => {
                    const blob = new Blob([response], { type: 'application/pdf' });
                    const url = URL.createObjectURL(blob);
                    const link = document.createElement('a');
                    link.href = url;
                    link.download = `preficha_${this.formData.apePaternoAsp}${this.formData.apeMaternoAsp}_Bachillerato.pdf`;
                    link.click();
