...
export class ViewPrimariaComponent implements OnInit {
  ...
  readDataServiceAspirantBasic() {
    this.serviceAspirantBasicService.getDataPrimaria().subscribe(
      {
        next: data => {
  ...
  genExcel(){
    this.serviceExcelBasicService.getDataBasicExcel("Primaria").subscribe(response => {
      ...
      link.download = `reporte_primaria_${day}.xlsx`;
      link.click();
